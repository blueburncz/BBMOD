...
{
	// Cloud shadow: read from alpha channel of the shadowmap at world-XY UV.
	// This is independent of the depth comparison UV stored in the RGB channels.
	...
	if (diskSize > 0.0)
	{
		// Area light: representative point method (Karis 2013).
		vec3 R = reflect(-V, N);
		float sinAlpha = sin(diskSize * 0.5);

		// Closest point on the light disc to the reflection ray.
		vec3 centerToRay = dot(L, R) * R - L;
		vec3 Lspec = normalize(L + centerToRay
			* clamp(sinAlpha / (length(centerToRay) + 0.0001), 0.0, 1.0));

		// Roughness adjustment: roughnessNew^4 = roughness^4 + sinAlpha^2.
		float a2 = m.Roughness * m.Roughness * m.Roughness * m.Roughness;
		float roughnessArea = sqrt(sqrt(a2 + sinAlpha * sinAlpha));
		...
		specular += shadowedColor * NdotLspec
			* xBRDF(m.Specular, roughnessArea, NdotLspec, NdotV, NdotH, VdotH);
		diffuse += shadowedColor * NdotL;
	}
	else
	{
...
void PBRShader(Material material, float depth)
{
	...
	if (bbmod_ShadowmapEnablePS == 1.0)
	{
		...
		bool isPoint = (shadowCasterIndex >= 0) && (BBMOD_GetPunctualLightDataB(shadowCasterIndex * 2).x == 0.0);
		...
		{
			vec3 position = BBMOD_GetPunctualLightDataA(shadowCasterIndex * 2).xyz;
