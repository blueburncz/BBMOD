function cm_torus_debug_draw_wireframe(torus, tex = -1, color = undefined, alpha = 1, mask = 0)
{
	if (mask > 0 && (mask & CM_TORUS_GROUP) == 0){return false;}
	
	var vbuff = global.cm_vbuffers_wf[CM_OBJECTS.TORUS];
	if (vbuff < 0)
	{
		vbuff = cm_create_torus_vbuff_wireframe(25, 12, 1, 1, c_white);
		global.cm_vbuffers_wf[CM_OBJECTS.TORUS] = vbuff;
	}
...
function cm_create_torus_vbuff_wireframe(hVerts, vVerts, hRep, vRep, color)
{
