function cm_capsule_debug_draw_wireframe(capsule, tex = -1, color = undefined, alpha = 1, mask = 0)
{
	if (mask > 0 && (mask & CM_CAPSULE_GROUP) == 0){return false;}
	
	var vbuff = global.cm_vbuffers_wf[CM_OBJECTS.CAPSULE];
	if (vbuff < 0)
	{
		vbuff = cm_create_capsule_vbuff_wireframe(20, 20, 1, 1, c_white);
		global.cm_vbuffers_wf[CM_OBJECTS.CAPSULE] = vbuff;
	}
...
function cm_create_capsule_vbuff_wireframe(hVerts, vVerts, hRep, vRep, color)
{
