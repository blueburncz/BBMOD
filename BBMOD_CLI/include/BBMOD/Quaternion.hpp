...
static inline void quaternion_from_axis_angle(quat_t q, const vec3_t axis, float angle)
{
	float halfAngle = angle * 0.5f;
	float sinHalfAngle = DSIN(halfAngle);
...
static inline void quaternion_multiply(quat_t R, const quat_t S)
{
	float Qx = R[3] * S[0] + R[0] * S[3] + R[1] * S[2] - R[2] * S[1];
	float Qy = R[3] * S[1] + R[1] * S[3] + R[2] * S[0] - R[0] * S[2];
	float Qz = R[3] * S[2] + R[2] * S[3] + R[0] * S[1] - R[1] * S[0];
	float Qw = R[3] * S[3] - R[0] * S[0] - R[1] * S[1] - R[2] * S[2];
	R[0] = Qx;
	R[1] = Qy;
	R[2] = Qz;
	R[3] = Qw;
