...
var _animation_duration = _animation[? "duration"];
var _animation_tics_per_sec = _animation[? "tics_per_sec"];
...
//show_debug_message(_animation_time);

var _transform = _bone[? "transform"];
var _bone_index = _bone[? "index"];

if (_bone_index >= 0)
{
	var _bone_data = ds_map_find_value(_animation[? "bones"], _bone_index);
	if (!is_undefined(_bone_data))
	{
		// Find position
		var _positions = _bone_data[? "positions"];
		...
		for (/**/; k < _positions_count - 1; ++k)
		{
			...
			if (_animation_time < _position_key_next[0])
			{
				break;
			}
		}
		...
		var _delta_time = _position_key_next[0] - _position_key[0];
		var _factor = (_animation_time - _position_key[0]) / _delta_time;
		_factor = clamp(_factor, 0, 1);
		var _position = ce_vec3_clone(_position_key[1]);
		ce_vec3_lerp(_position, _position_key_next[1], _factor);
		...
		// Find rotation
		var _rotations = _bone_data[? "rotations"];
		var _rotations_count = ds_list_size(_rotations);
		...
		for (/**/; k < _rotations_count - 1; ++k)
		{
			...
			if (_animation_time < _rotation_key_next[0])
			{
				break;
			}
		}
		...
		var _delta_time = _rotation_key_next[0] - _rotation_key[0];
		var _factor = (_animation_time - _rotation_key[0]) / _delta_time;
		_factor = clamp(_factor, 0, 1);
		var _rotation = ce_quaternion_clone(_rotation_key[1]);
		ce_quaternion_slerp(_rotation, _rotation_key_next[1], _factor);
...
if (_bone_index >= 0)
{
	var _final_transform = _bone[? "offset"];
